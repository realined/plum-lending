...
Current checkpoint: the complete technical live baseline passes. Reader OAuth/private-app authentication, approved identities, separate writer consent, live seed/replay, associations, actual AI interpretation, reviewed browser execution, full threads and downloaded CSV are verified. PostgreSQL 17.11 and the separate worker are running. API credentials are privately populated; the owner authorized up to $1 testing/rehearsal usage and confirmed funded credit with auto-reload off. Preserve the existing `.env` and encryption key. See [measured acceptance](live-acceptance.md) and [baseline report](baseline-report.md). No deployment or publication is authorized.
...
Live interpretation requires `OPENAI_API_KEY`, entered locally by the owner. This setup now has a privately supplied, seven-day restricted Responses key and uses `OPENAI_MODEL=gpt-4.1-mini`. The owner approved up to $1 total model usage for acceptance/rehearsal and reports API credit funded with auto-reload off. This is an operator limit, not a product-wide hard billing cap. Do not change models, buy further credits or enable auto-reload without review. ChatGPT subscription credit is separate from API billing. On another installation, arrange API credentials/funding before live interpretation. See [model pricing](https://developers.openai.com/api/docs/models/gpt-4.1-mini).

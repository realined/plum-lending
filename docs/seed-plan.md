...
Run `pnpm seed:preview` to reproduce the proposal. It loads no environment file and has no network/database imports. Add `--expanded` to the underlying dry-run script for the reviewed 20-contact expansion; other arguments are rejected. The default frozen reference time is `2026-09-16T12:00:00.000Z`, so the qualifying interval is `[2024-09-16T12:00:00.000Z, 2026-06-16T12:00:00.000Z)`. The output includes a SHA-256 digest of the complete proposal. A changed namespace, date or record set requires a new review; the actual live request's frozen window must be checked before comparing results.
...
After owner execution approval and current writer consent, stop web and worker, then invoke the existing CLI with `expand --approve=<expanded-digest>` under the same private environment loading and explicit enable flag as `seed:apply`. The command verifies the complete original corpus without dataset writes, privately backs up its journal to `pre-expansion-state.json`, atomically transitions only the approved plan digest, preserves original IDs, and records each new write. It publishes the expanded reader receipt only after complete verification. Repeating `expand` resumes/replays the expanded plan while comparing every original ID against the backup. Pending uncertain creates still block replay. Do not manually reset journals.

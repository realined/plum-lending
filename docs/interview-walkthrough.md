...
Live evidence: real Gmail OAuth, HubSpot private-app access, separate controlled seed creation/replay, five OpenAI interpretation cases and the full browser → worker → CSV workflow pass. The representative browser run scanned 7 contacts and returned exactly 2 contacts, 2 threads and 5 messages in 7.087 seconds. The downloaded eight-column CSV matches stored output. Earlier typed-specification runs completed in 6.862/7.402 seconds. All data in these service accounts used for the demonstration is approved fictional content; unrelated mail stays outside the receipt boundary.

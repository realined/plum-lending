...
Setup evidence: a dedicated Google Cloud project and Gmail API enablement are verified, and a private local environment file has been generated. OAuth app registration, one test user and the read-only scope are verified; the owner reports client creation and private credential entry, and non-disclosing checks confirm the fields are populated. The owner completed the real Gmail read-only grant. The app callback verified the approved profile and saved an encrypted connection; no ingestion has run. The owner-created HubSpot token passed the adapter’s real read-only authentication check, and the existing server connection function saved it encrypted in fresh PostgreSQL. The authenticated live UI shows both providers Connected and a separate worker heartbeat; account identity and ingestion remain separate pending checks. Enabling an API does not grant access to a mailbox.
...
HubSpot onboarding and the exact `contact_type` schema are verified. The owner created the read-only app and saved its credential; API authentication passed across the five expected read endpoints. The connection has also been encrypted into local PostgreSQL and verified in the live UI. This does not yet prove association correctness or CSV results. The current HubSpot wizard labels private apps as legacy and recommends Service Keys. Keeping the already implemented private-app flow limits changes during acceptance; its future-support limitation is migration debt to revisit before production.
...
**How do you know private emails cannot leak into this demonstration?** Automated tests inject unknown IDs, accidentally labeled real-message stand-ins and mid-read replies, then assert no unapproved body request occurs. Worker tests prove boundary failures publish neither canonical records nor export rows. A positive test preserves the full approved thread, including out-of-window context, through CSV. These 30 new tests supplement existing coverage; actual OAuth/provider behavior still requires live verification.

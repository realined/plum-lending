...
Status: the technical live baseline has passed. Distinguish live positive-path proof from automated mocked failure coverage, and use `baseline-report.md` and `live-acceptance.md` for the exact evidence and limits.

Live evidence: real Gmail OAuth, HubSpot private-app access, controlled seed expansion/replay, five original OpenAI semantic cases and fresh expanded browser → worker → CSV workflow pass. The current corpus is 20 contacts, 20 companies, 8 deals, 21 threads and 59 messages. The expanded browser run scanned 20 contacts and returned exactly 10 contacts, 12 threads and 45 messages in 22.869 seconds, with 10 exclusions and zero failures. Its downloaded eight-column CSV matches stored output. The original seven-contact baseline remains historical evidence. All demo content is fictional; unrelated mail stays outside the receipt boundary. See `expanded-live-acceptance.md`.
...
The private-app token has read permissions only. Connection validation checks objects, Sponsor property and pipelines. The explicit `pnpm hubspot:check` command reuses those bounded reads before seeding, without needing Gmail, PostgreSQL or OpenAI; it emits fixed safe messages and does not persist a connection. Successful authentication is weaker evidence than correct account identity, associations or complete ingestion. Object and v4 association pagination are explicit, including repeated-cursor protection. The contact's primary email stays first, additional identities are normalized, and configured Sponsor matching cannot accidentally accept another raw role value.
...
## Dedicated live baseline and synthetic seeding

The owner revised account selection: the next acceptance environment uses the existing job-search/BenTech Gmail account in seed-only mode, plus a new free HubSpot instance populated with fictional records. This still exercises real OAuth, authorization scopes, provider pagination, CRM associations, Gmail threading and CSV generation. Contract mocks prove local behavior; controlled records in real services prove the integration. Neither replaces the other.

At the historical Phase 0 checkpoint, accounts were unconfigured and the live seeder did not exist. Both reader connections now work. The seven-case proposal, MIME generator, offline dry-run, separate OAuth helper, guarded writer, durable replay journal and cleanup are implemented. Latest checks pass 264 tests, lint, types and build. Separate writer credentials/consent, preflight, seeding, replay and live reader/CSV checks now pass. Natural-language interpretation now passes five real semantic cases and the complete browser workflow. See `seed-plan.md`. See `phase-0-setup-checklist.md`. The seed writer must have its own authorization, an explicit enable flag, verified account identity, stable namespace, dry-run, reconciliation and scoped cleanup. It never runs at app startup. Successful replay reuses journaled provider IDs; an uncertain create stops for manual inspection instead of risking a duplicate. Identical Message-ID headers alone are not deduplication. The owner chose insert/read/labels: cleanup disables the receipt and removes the seed label, retaining Gmail messages. This narrower authorization avoids a send-capable scope. Live seed replay reused the same provider IDs without duplicate creates.

Explain the distinction between environment readiness, validated credentials, successful ingestion and correct expected results. The HubSpot environment token currently validates and persists when a job starts; the new independent pre-seed read-only check is implemented, tested offline and has now passed against the owner’s account using the privately saved token. Live interpretation has a metered API dependency; the owner authorized up to $1 and supplied a restricted key privately. The five instrumented semantic calls cost an estimated $0.0009096; the UI call’s exact usage was not captured. The baseline report precedes any new feature/refinement work.

HubSpot onboarding and the exact `contact_type` schema are verified. The owner created the read-only app and saved its credential; API authentication passed across the five expected read endpoints. The connection has also been encrypted into local PostgreSQL and verified in the live UI. Subsequent live seed and reader checks also verified reviewed associations and CSV results. The current HubSpot wizard labels private apps as legacy and recommends Service Keys. Keeping the already implemented private-app flow limits changes during acceptance; its future-support limitation is migration debt to revisit before production.

## Protecting unrelated correspondence during a live demonstration

The ordinary Gmail adapter preserves whole threads, which means a search label cannot by itself protect unrelated replies. Protected mode uses the exact insertion receipt from the separate seeder, verifies the OAuth profile, lists only seed-labeled thread IDs without snippets, checks membership without content, then reads each approved message ID individually. This avoids fetching a real reply that arrives between a membership check and a full-thread request. Mixed threads fail the entire run; we do not silently remove private messages and call the remainder complete.

The same deterministic executor, normalizer and CSV generator still operate on real API responses. The scope restriction defines the controlled acceptance corpus rather than changing Sponsor/date/deal eligibility. Google still grants mailbox-wide read permission, so application restrictions are not token-level isolation. Local manifest integrity and protection of the server/token remain assumptions.

**How do you know private emails cannot leak into this demonstration?** Automated tests inject unknown IDs, accidentally labeled real-message stand-ins and mid-read replies, then assert no unapproved body request occurs. Worker tests prove boundary failures publish neither canonical records nor export rows. A positive test preserves the full approved thread, including out-of-window context, through CSV. These 30 new tests supplement existing coverage; actual OAuth and the protected positive reader flow now pass; adversarial privacy cases remain automated synthetic tests.

## Integration lessons to defend

**What did real accounts catch that mocks missed?** HubSpot rejected the fictional .test contact domain. With owner approval we used namespaced reserved @example.com contact addresses and disabled automatic company association, preserving the reviewed truth set. The write journal allowed reconciliation of the one known created company without duplicates.

**Why did repeat CSV bytes differ?** Gmail returned a different opaque attachment retrieval ID for the same message attachment. Every other field matched; canonical entity counts stayed constant. Provider retrieval handles are not a semantic-content fingerprint. The raw communication JSON preserves provenance, so claim stable eligibility/content and idempotent entities, not byte-identical raw exports.

**What is actually live and what is still mocked?** Both service connections, synthetic corpus insertion, ingestion, associations, deterministic eligibility, complete threads and CSV download are live-verified. Adversarial boundaries and cleanup are tested with controlled mocks. Live AI interpretation now passes, with actual structured-output provenance shown in the UI.

## Verified presentation sequence

Lead with the live product: “I built a relationship-segmentation tool that combines CRM eligibility with complete email conversations. AI converts a plain-language request into a strict specification that the administrator reviews. Deterministic code reads Gmail and HubSpot, applies explainable rules, and exports complete threads. The live demonstration uses seven fictional contacts in real accounts; it returns exactly two contacts and five messages, with a verified CSV in about seven seconds.”

Then show the representative request, reviewed Sponsor/inbound/date/Closed Won criteria, background completion, exclusion counts, four-message Cedar conversation and CSV. Explain why the recent August reply remains despite the June eligibility cutoff. Explain why a Closed Lost deal permits the other contact. Do not present the five-case model evaluation as a broad language benchmark or seven contacts as a scale test.

Use the failed .test insertion and changing attachment retrieval ID as concrete integration lessons: mocked contracts cannot establish all provider validation behavior; stable business results do not guarantee byte-identical raw provider handles.

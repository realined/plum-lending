...
import { authenticated, constantEqual, cookieOptions, readBody, requireAuth, requireOrigin, signSession } from "@/server/auth";
import { config } from "@/server/config";
import { googleAccessToken, liveProviders, saveConnection } from "@/server/connections";
...
vi.mock("postgres", () => ({ default: vi.fn(() => { throw new Error("External PostgreSQL is forbidden in security unit tests"); }) }));
...
function flipEncodedByte(value: string) { const bytes = Buffer.from(value, "base64url"); bytes[0] ^= 1; return bytes.toString("base64url"); }
const storedToken = (access = "synthetic-access-token", expiresAt = now + 3600_000) => JSON.stringify({ access_token: access, refresh_token: "synthetic-refresh-token", expires_at: expiresAt });
...
afterEach(() => { vi.restoreAllMocks(); vi.unstubAllGlobals(); vi.unstubAllEnvs(); singleton.plumDB = previousSingleton; });
...
describe("authenticated token encryption", () => {
  it("round-trips Unicode credentials with a fresh nonce for every encryption", () => {
    ...
    const first = encryptToken(plaintext), second = encryptToken(plaintext);
  ...
  it.each([1, 2, 3])("rejects tampering with encrypted envelope component %i", index => {
  ...
  it("rejects a different encryption key and malformed envelopes", () => {
    ...
    for (const malformed of ["", "plaintext", "v2.a.b.c", "v1..b.c", "v1.a..c"]) expect(() => decryptToken(malformed)).toThrow();
  });
  it.each(["", "invalid-base64", Buffer.alloc(16).toString("base64")])("rejects an absent or wrong-length key", key => {
    vi.stubEnv("TOKEN_ENCRYPTION_KEY", key);
    expect(() => encryptToken("Synthetic credential")).toThrow("TOKEN_KEY_REQUIRED");
  });
});
...
describe("live session authentication and request boundaries", () => {
  it("accepts an unexpired signature and expires it at the exact expiration instant", () => {
    ...
    expect(() => requireAuth(request(session))).toThrow(expect.objectContaining({ status: 401 }));
  });
  it("rejects missing, malformed, modified, and incorrectly signed cookies", () => {
    ...
    for (const invalid of [undefined, "", "garbage", `${Number(expiration) + 1000}.${signature}`, `${expiration}.${badSignature}`, `${signed}.suffix`, "Infinity.invalid", "NaN.invalid"]) expect(authenticated(request(invalid))).toBe(false);
    vi.stubEnv("SESSION_SECRET", randomBytes(32).toString("base64url"));
  ...
  it("requires an exact configured origin on mutations", () => {
    ...
    for (const origin of [null, "null", "https://other.example.test", `${appUrl}.other.example.test`, "http://plum.example.test"]) expect(() => requireOrigin(request(undefined, origin))).toThrow(expect.objectContaining({ status: 403 }));
  });
  it("sets browser session cookies to HttpOnly, SameSite and Secure on HTTPS", () => {
    expect(cookieOptions()).toEqual({ httpOnly: true, secure: true, sameSite: "lax", path: "/", maxAge: 28_800 });
    vi.stubEnv("APP_URL", "http://localhost:3000");
    expect(cookieOptions().secure).toBe(false);
  });
  it("accepts JSON and rejects malformed, wrong-type, and oversized UTF-8 bodies", async () => {
    const bodyRequest = (body: string, headers: Record<string, string> = { "content-type": "application/json" }) => new NextRequest(`${appUrl}/api/segments`, { method: "POST", headers, body });
    expect(await readBody(bodyRequest('{"query":"synthetic"}'))).toEqual({ query: "synthetic" });
    await expect(readBody(bodyRequest("bad json"))).rejects.toMatchObject({ status: 400 });
    await expect(readBody(bodyRequest("{}", { "content-type": "text/plain" }))).rejects.toMatchObject({ status: 415 });
    await expect(readBody(bodyRequest("{}", { "content-type": "application/json", "content-length": "12001" }))).rejects.toMatchObject({ status: 413 });
    await expect(readBody(bodyRequest(JSON.stringify({ data: "é".repeat(6000) })))).rejects.toMatchObject({ status: 413 });
  });
});

describe("live configuration and data isolation", () => {
  it.each(["", ":memory:", ".data/demo", "sqlite://local", "https://database.example.test", "postgres-pretend://example.test"])("rejects a non-PostgreSQL live database URL: %s", databaseUrl => {
  ...
  it("accepts PostgreSQL schemes, requires live secrets and HTTPS, and disables the embedded worker by default", () => {
    ...
    vi.stubEnv("DATABASE_URL", "postgres://synthetic:synthetic@database.example.test/plum_test");
  ...
  it("uses an isolated demo database even when a PostgreSQL DATABASE_URL is present", async () => {
    const directory = await mkdtemp(path.join(os.tmpdir(), "plum-security-demo-"));
    vi.stubEnv("APP_MODE", "demo");
    ...
    try {
      ...
      expect(await isolated.query("SELECT count(*)::int AS count FROM connections")).toEqual([{ count: 0 }]);
    } finally {
...
describe("connection ownership and token refresh fencing", () => {
  it("stores encrypted credentials and refuses to silently overwrite an existing connection", async () => {
    ...
    const [before] = await database.query<{ encrypted_token: string; generation: string; mailbox: string }>("SELECT encrypted_token,generation,mailbox FROM connections WHERE id='gmail'");
    ...
    await expect(saveConnection("gmail", storedToken("different-synthetic-token"), "second@example.test")).rejects.toThrow();
    expect(await database.query("SELECT encrypted_token,generation,mailbox FROM connections WHERE id='gmail'")).toEqual([before]);
  });
  it("uses a valid cached token and rejects a stale connection generation before network access", async () => {
    await saveConnection("gmail", storedToken(), "first@example.test");
    const [row] = await database.query<{ generation: string }>("SELECT generation FROM connections WHERE id='gmail'");
    expect(await googleAccessToken(row.generation)).toBe("synthetic-access-token");
    await expect(googleAccessToken("different-generation")).rejects.toThrow("CONNECTION_CHANGED");
    expect(fetchMock).not.toHaveBeenCalled();
  });
  it("refreshes an expired token without losing an omitted refresh token", async () => {
    await saveConnection("gmail", storedToken("old-synthetic-access", now - 1000), "first@example.test");
    fetchMock.mockResolvedValueOnce(jsonResponse({ access_token: "new-synthetic-access", expires_in: 3600 }));
    expect(await googleAccessToken()).toBe("new-synthetic-access");
    const [row] = await database.query<{ encrypted_token: string }>("SELECT encrypted_token FROM connections WHERE id='gmail'");
    expect(JSON.parse(decryptToken(row.encrypted_token))).toEqual({ access_token: "new-synthetic-access", refresh_token: "synthetic-refresh-token", expires_at: now + 3600_000 });
  ...
  it("rejects a refresh result after the mailbox connection is replaced", async () => {
    ...
    await saveConnection("gmail", storedToken("replacement-synthetic-access"), "second@example.test");
    finishRefresh(jsonResponse({ access_token: "stale-refresh-result", expires_in: 3600 }));
    await rejected;
  ...
  it("reuses a peer's successful refresh for the same connection generation", async () => {
    ...
    const first = googleAccessToken(), second = googleAccessToken();
    await vi.waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(2));
    finish[0](jsonResponse({ access_token: "winning-synthetic-access", expires_in: 3600 }));
    expect(await first).toBe("winning-synthetic-access");
    finish[1](jsonResponse({ access_token: "superseded-synthetic-access", expires_in: 3600 }));
  ...
  it("fences a captured Gmail provider after reconnecting to a different mailbox", async () => {
    ...
    await database.query("DELETE FROM connections WHERE id='gmail'");
    await saveConnection("gmail", storedToken("replacement-synthetic-access"), "second@example.test");
    await expect(providers.email.validateConnection()).rejects.toThrow("CONNECTION_CHANGED");
    expect(fetchMock).not.toHaveBeenCalled();
  });
});
